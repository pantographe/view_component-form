...
RSpec.describe ViewComponent::Form::Builder, type: :builder do
  ...
  shared_examples 'the default form builder when calling' do |method_name, params|
    subject { form.public_send(method_name, *params) }
    let(:default_form_builder) { form_with(object, builder: ActionView::Helpers::FormBuilder) }
    let(:default_form_builder_output) { default_form_builder_output.public_send(method_name params) }

    it { is_expected.to eq(default_form_builder.public_send(method_name, *params)) }
  end

  it_behaves_like 'the default form builder when calling', :check_box, ["validated"]
  it_behaves_like 'the default form builder when calling', :check_box, ["gooddog", {}, "yes", "no"]
  it_behaves_like 'the default form builder when calling', :check_box, ["accepted", { class: 'eula_check' }, "yes", "no"]
